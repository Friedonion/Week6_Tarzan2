...
struct FActorSaveData
{
    ...
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(FActorSaveData, ActorID, ActorClass, ActorLabel, bTickInEditor, RootComponentID, Components)
};
...
FSceneData SceneManager::WorldToSceneData(const UWorld& InWorld)
{
    ...
    for (const auto& Actor : Actors)
    {
        ...
        actorData.bTickInEditor = Actor->IsActorTickInEditor() ? TEXT("true") : TEXT("false"); // bEditorTick 속성 추가
...
bool SceneManager::LoadWorldFromData(const FSceneData& sceneData, UWorld* targetWorld)
{
    ...
    for (const FActorSaveData& actorData : sceneData.Actors)
    {
        ...
        SpawnedActor->SetActorTickInEditor(actorData.bTickInEditor.ToBool());
